/// <summary>
/// AudioInstance
/// Unityで足りない制御(フェード、ランダムピッチなど)をプラスした再生クラス
/// </summary>
...
namespace USnd
{
    public partial class AudioManager : MonoBehaviour
    {
        private class AudioInstance
        {
            ...
            int currentPitch = 0;
            float currentVolume = 0;            // 純粋なインスタンスのボリューム、係数、ctrlを含まない.

            float volumeFactor = 1.0f;          // ボリューム係数をいれておく.
            float ctrlVolumeFactor = 1.0f;      // ポーズ時のフェードなどのボリューム管理につかう.
            ...
            AudioParamUpdater controlUpdater = new AudioParamUpdater(); // ポーズ制御用のボリューム管理

            int prevPlaySamples = 0;

            bool onPause = false;

            int randomIndex = 0;        // ランダム再生インスタンスだったときの添え字
            ...
            Transform CacheSourceTransform { get { return (_cacheSourceTransform != null) ? _cacheSourceTransform : (_cacheSourceTransform = source.transform); } }
            Transform _cacheSourceTransform;
            ...
            public void Init(AudioSource playSource, float factor, int index)
            {
                ...
                source.gameObject.transform.position = defaultPos;
            }
            ...
            public void Update()
            {
                if (source != null)
                {
                    ...
                    {
                        source.gameObject.transform.position = trackingObject.transform.position;
                    }
                    if ( targetTransform != null )
                    {
                        source.gameObject.transform.position = targetTransform.position;
                    }

                    // delay中、ポーズ中はパラメータ更新しないので、値推移中にポーズがかかったら続きからになる.
                    // 再生中 and 再生位置が前の値と一緒 and ポーズしていないときに値更新.
                    if (source.isPlaying && source.timeSamples != prevPlaySamples && onPause == false)
                    {
                        ...
                        if (pitchUpdater.active)
                        {
                            ...
                            if (isAudioDebug)
                            {
                                //AudioDebugLog.Log ("pitch: " + currentPitch);
                            }
                            if (isAudioDebug)
                            {
            ...
            public void Play(float delay)
            {
                ...
                // Prepare後間が空いているかもしれないのでスタート時間を更新してから再生開始する
                volumeUpdater.UpdateStart();
                panUpdater.UpdateStart();
                pitchUpdater.UpdateStart();
            ...
            {
                CacheSourceTransform.position = position;
            }
            ...
            // ----------------------------------------------------------------------
            // Start random or default volume.
