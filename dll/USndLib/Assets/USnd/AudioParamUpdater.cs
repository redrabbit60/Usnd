    ...
    public partial class AudioManager : MonoBehaviour
    {
        private class AudioParamUpdater
        {
            ...
            public void SetParam(float _start, float _target, float moveTime, bool isLow)
            {
                ...
                // アップデート開始タイムを更新、Set後すぐUpdateが来る場合はこれは呼ばなくてもOK
            public void UpdateStart()
            {
                // 処理時間がそれなりにかかるのでactiveがtrueのときだけ実行
                if (active == true)
                {
                    prevTime = Time.unscaledTime;
                }
            ...
            public float Update()
            {
                if (active == true)
                {
                    ...
                    current += (unit * (currentTime - prevTime));//Time.deltaTime);
