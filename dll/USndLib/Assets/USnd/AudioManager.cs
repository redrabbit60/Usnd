...
namespace USnd
{
    ...
    public partial class AudioManager : MonoBehaviour
    {
		...
		// ---------------------------------------------------------------
		// 3Dサウンド設定されているラベルを強制的に2Dで再生する(PlayOptionと同じ引数Ver).
		static public int Play2D(string labelName, float volume, float fadeTime, float pan, int pitch, float delay = -1)
		{
			if (manager != null)
			{
				return manager.play2D(labelName, volume, fadeTime, pan, pitch, delay);
			}
			return AudioDefine.INSTANCE_ID_ERROR;
		}
        ...
        static public float GetLabelVolume(string labelName)
        {
            if (manager != null)
            {
                return manager.getLabelVolume(labelName);
            }
            return 0;
        }
		...
		static public void OffPauseCategory(string categoryName, List<int> instanceList, float fadeTime = -1)
		{
			if (manager != null)
			{
				manager.offPauseCategory(categoryName, instanceList, fadeTime);
			}
		}
		...
		// ---------------------------------------------------------------
		// 指定したラベルがインターバル期間中か(true:インターバル中　false:インターバル中ではない)
		static public bool IsInterval(string labelName)
		{
			if (manager != null)
			{
				return manager.isInterval(labelName);
			}
			return false;
		}
		...
		// ---------------------------------------------------------------
		// 現在発音中の総数を取得
		static public int GetCurrentPlayNum()
		{
			if (manager != null)
			{
				return manager.getCurrentPlayNum();
			}
			return 0;
		}
		...
		static public void UpdateUnityMixerName(string labelName, string newMixerName)
		{
			if (manager != null)
			{
				manager.updateUnityMixerName(labelName, newMixerName);
			}
			return;
		}
		...
		static public void UpdateSpatialGroupName(string labelName, string newGroupName)
		{
			if (manager != null)
			{
				manager.updateSpatialGroupName(labelName, newGroupName);
			}
			return;
		}
